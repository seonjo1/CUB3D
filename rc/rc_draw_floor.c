...
void	rc_floor_draw_pixel(t_data *data, int x, int y)
{
	...
	f_data.camera_x = 2 * x / (double) WIN_WIDTH - 1;
	...
	f_data.floor_x = data->player.pos.x + f_data.ray.x * (f_data.camera_y / 2) / y;
	f_data.floor_y = data->player.pos.y + f_data.ray.y * (f_data.camera_y / 2) / y;
	if (f_data.floor_x >= 0 && f_data.floor_y >= 0 && f_data.floor_x < data->map.col && f_data.floor_y < data->map.row)
	{
		...
		utils_draw_point(data, x, y + WIN_HEIGHT / 2, f_data.color);
	}
}
